...
const REMINDER_SNAPSHOT = 'monthly_snapshot';
...
function updateLeadGoal(goal) {
    const store = getStore();
    store.leads.quarterlyGoal = goal;
    saveStore(store);
}

function addLeadEntry(entry) {
    const store = getStore();
    entry.id = Date.now().toString();
    entry.date = entry.date || new Date().toISOString();
    entry.type = 'lead';
...
function addMetricSnapshot(snapshot) {
    const store = getStore();
...
function deleteMetricSnapshot(id) {
    const store = getStore();
...
function applyStripePreviewParam() {
...
async function connectStripeKey(apiKey) {
...
async function autoSyncStripeIfDue() {
    try {
...
function proCardHeading(featureKey, heading) {
...
function proTeaser(featureKey, heading, hint, action) {
    if (isProUser() && isFeatureLive(featureKey)) return '';
...
function renderTooltip(whatStr, whyStr, position = 'top', labels = {}) {
